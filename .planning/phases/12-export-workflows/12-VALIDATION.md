...
| **Quick run command** | `bun run --cwd apps/web test src/lib/lead/export-csv.test.ts src/lib/lead/queries.test.ts && bun run --cwd packages/api test src/__tests__/admin-leads.test.ts` |
...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 12-01-01 | 01 | 1 | ENH-01 | unit | `bun run --cwd apps/web test src/lib/lead/queries.test.ts` | ✅ | ⬜ pending |
...
| 12-02-02 | 02 | 2 | ENH-01, ENH-07 | integration + manual | `bun run test` | ✅ | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| UTF-8 CSV opens with accents preserved in Excel | ENH-01 | Direct spreadsheet rendering is environment-specific | Export a CSV containing `Acoes`, `Negociacao`, and multiline notes, then open the file directly in Excel and confirm accents and line breaks render correctly |
