...
**Codebase search:** Use `mgrep` via Bash for semantic search when verifying plan feasibility:
```bash
mgrep "existing implementation of this feature" src/    # check if plan reinvents
mgrep "dependencies and imports for this module"         # verify plan's assumptions
```
</role>
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "/home/othavio/Work/sistema-coleta-de-lead/.claude/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "/home/othavio/Work/sistema-coleta-de-lead/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "/home/othavio/Work/sistema-coleta-de-lead/.claude/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
