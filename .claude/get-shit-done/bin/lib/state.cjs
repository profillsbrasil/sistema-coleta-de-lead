...
function cmdStateAdvancePlan(cwd, raw) {
	...
	if (legacyPlan && legacyTotal) {
		currentPlan = Number.parseInt(legacyPlan, 10);
		totalPlans = Number.parseInt(legacyTotal, 10);
	} else if (planField) {
		// Compound format: "2 of 6 in current phase" or "2 of 6"
		currentPlan = Number.parseInt(planField, 10);
		const ofMatch = planField.match(/of\s+(\d+)/);
		totalPlans = ofMatch ? Number.parseInt(ofMatch[1], 10) : Number.NaN;
...
function cmdStateSnapshot(cwd, raw) {
	...
	const totalPhases = totalPhasesRaw
		? Number.parseInt(totalPhasesRaw, 10)
		: null;
	const totalPlansInPhase = totalPlansRaw
		? Number.parseInt(totalPlansRaw, 10)
		: null;
	const progressPercent = progressRaw
		? Number.parseInt(progressRaw.replace("%", ""), 10)
		: null;
...
function buildStateFrontmatter(bodyContent, cwd) {
	...
	if (progressRaw) {
		...
		if (pctMatch) {
			progressPercent = Number.parseInt(pctMatch[1], 10);
		}
	}
