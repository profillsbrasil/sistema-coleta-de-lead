...
- **Observabilidade (C2):** helper `recordAlert(event, severity, payload)` em `alerts.ts` faz best-effort INSERT em `whatsapp.alerts` (failure de write em alerta nunca derruba caller). Chamado em `outbound_blocked_outside_window`, `outbound_failed_dead_letter`, `outbound_failed_permanent`/`outbound_failed_status` (status webhook) e `code_generation_exhausted`. Admin: `/admin/whatsapp/alerts` lista pendentes + marca como lido; `/admin/whatsapp/health` mostra resumo 24h (msgs in/out, drop rate, leads por estado, top failed codes, pricing por categoria). Cron de quality_rating snapshot é tech-debt #38.
- **LGPD (C1 ajustado):** sem cron de retention automática. `welcome()` mostra link da política se `whatsapp.config.privacy_policy_url` estiver setada. No consent (accept), grava `participants.terms_url_snapshot = config.privacy_policy_url` junto com `terms_version`. DSR endpoint: `/admin/whatsapp/dsr` → mutation `whatsapp.dsrDelete` faz hard delete cascade (participant → messages via FK cascade, rate_limit por waId) dentro de transação que também grava `whatsapp.dsr_audit` (waId, deleted_by, deleted_at, reason, participant_snapshot). Não há undo.
...
- **Status webhooks (B4):** POST despacha `after(processStatusAsync)` quando `value.statuses[]` chega. UPDATE `messages` WHERE `wamid` = status.id com `delivered_at`/`read_at`/`failed_at`/`failed_code`/`failed_reason`/`pricing_category`/`pricing_billable`. `failed` com code permanente (131026/131047) gera alerta `outbound_failed_permanent`. Replay automático de retentáveis fica como dívida — ver tech-debt #37.
...
- **Fluxo guiado de tarefas (Opção B):** após `AWAITING_COMPANY` válido, lead entra em **`AWAITING_TASK_1` → `_2` → `_3` → `_POST` → `COMPLETED`**. Cada substate manda um `taskStep` (CTA URL "Abrir Instagram") + `taskStepConfirm` (reply button `task_done_N`). `tasksIntro` é enviado uma vez na entrada. `taskProgress` é atualizado incrementalmente (`follow_N=true` a cada step, `comment=true` no post). Indicador `(n/4)` no body de cada step. Texto livre / botão errado em qualquer substate → `taskNudge` com cooldown anti-spam (`isWithinResponseCooldown`). `AWAITING_TASKS` (state legacy) ainda existe no enum: aceita o botão antigo `tasks_done` (atalho pra COMPLETED) e em qualquer outra mensagem reabre o fluxo novo a partir de `AWAITING_TASK_1`. Coluna `state` em `whatsapp.participants` é `text` (sem constraint), então não há migration necessária. Os 4 novos substates também estão em `INTERMEDIATE_STATES` → TTL 24h. Flavor da copy ("Para começar, siga…", "Agora siga…", "E também siga…") vive em `state-machine.ts:TASK_INTRO_BY_INDEX`, não em `messages.ts` — a função `taskStep` é genérica. Button IDs canônicos: `TASK_STEP_BUTTON_IDS` em `messages.ts` (`task_done_1/2/3/post`).
