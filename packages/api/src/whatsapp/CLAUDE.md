...
- **Observabilidade (C2):** helper `recordAlert(event, severity, payload)` em `alerts.ts` faz best-effort INSERT em `whatsapp.alerts` (failure de write em alerta nunca derruba caller). Chamado em `outbound_blocked_outside_window`, `outbound_failed_dead_letter`, `outbound_failed_permanent`/`outbound_failed_status` (status webhook), `code_generation_exhausted` e `quality_rating_degraded` (cron). Admin: `/admin/whatsapp/alerts` lista pendentes + marca como lido; `/admin/whatsapp/health` mostra resumo 24h (msgs in/out, drop rate, leads por estado, top failed codes, pricing por categoria) + último snapshot Meta (quality_rating + tier).
- **Cron quality snapshot:** `/api/cron/whatsapp-quality-snapshot` (configurado em `/vercel.json`, 06:00 UTC) puxa `quality_rating` + `messaging_limit_tier` do Graph API (`GET /vXX/{PHONE_NUMBER_ID}?fields=...`), grava em `whatsapp.health_snapshots`. Se rating cair (HIGH/GREEN → MEDIUM/YELLOW → LOW/RED), dispara `quality_rating_degraded` (severity high). Auth: `Authorization: Bearer ${env.CRON_SECRET}` (Vercel injeta automaticamente em prod). Em dev sem `CRON_SECRET`, requests não-bearer são aceitos (não bloqueia desenvolvimento local).
...
- **Status webhooks (B4):** POST despacha `after(processStatusAsync)` quando `value.statuses[]` chega. UPDATE `messages` WHERE `wamid` = status.id com `delivered_at`/`read_at`/`failed_at`/`failed_code`/`failed_reason`/`pricing_category`/`pricing_billable`. `failed` com code permanente (131026/131047) gera alerta `outbound_failed_permanent`. `failed` retentável dispara `tryReplayOutbound` — cap em 1 replay por mensagem (controlado por `messages.replay_count`) + guard rail 23h. Nova outbound row herda `replay_count + 1` e `replay_of_message_id = original.id`. Replay-failed permanente vira dead-letter com prefixo `replay_failed:` no reason.
