...
function cmdInitPlanPhase(cwd, phase, raw) {
	...
	const result = {
		...
		has_reviews: phaseInfo?.has_reviews,
		...
		function findCodeFiles(dir, depth) {
			...
			for (const entry of entries) {
				...
				if (
					...
					findCodeFiles(path.join(dir, entry.name), depth + 1)
				) {
					return true;
				}
			}
		...
		try {
			...
			if (dirMatch) {
				...
				if (newestMtime > 0) {
					...
					isActive = now - newestMtime < 300_000; // 5 minutes
				}
			}
		} catch {
			/* intentionally empty */
	...
	try {
		...
		const dirs = entries
			...
			.sort((a, b) => {
				...
				return Number.parseInt(pa[1], 10) - Number.parseInt(pb[1], 10);
			});
