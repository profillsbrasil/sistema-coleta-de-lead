...
function loadConfig(cwd) {
	...
	const defaults = {
		...
		context_window: 200_000, // default 200k; set to 1000000 for Opus/Sonnet 4.6 1M models
...
function withPlanningLock(cwd, fn) {
	...
	const lockTimeout = 10_000; // 10 seconds
...
function searchPhaseInDir(baseDir, relBase, normalized) {
	try {
		...
		const match = dirs.find((d) => {
			...
			if (d.toUpperCase().startsWith(normalized.toUpperCase())) {
				return true;
			}
			return false;
		});
